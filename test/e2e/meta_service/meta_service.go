...
var _ = Describe("MetaService", func() {
	var (
		...
		driver           *FakeNFSDriver
		driverSocketPath string
		driverDumpYard   string
	)
	...
	BeforeEach(func() {
		...
		Expect(err).To(BeNil())
		driverSocketPath = filepath.Join(currentDir, "nfs_driver")
		driverDumpYard = currentDir
		driver = NewFakeNFSDriver(driverSocketPath, driverDumpYard)
		go driver.RunDriver()

		// default config
		config := options.NewMetaServiceFlags()

		// initialize backup repository
		repository, err := repo.NewBackupRepository(driverSocketPath)
		Expect(err).To(BeNil())
